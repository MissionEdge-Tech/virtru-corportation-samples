...
This chart was created by Kompose from the original docker-compose.cop-web-server.yaml

By transfering the COP Web Server deployment into a Kubernetes deployment the program is isolated from the backend database, DSP, and Keycloak services to better replicate a production environment. For local development purposes, these remaining services are still spun up within Docker containers using the same docker-compose files.

At the time of writing, the deployment of the COP service is still dependent upon the sucessful deployment of these other services. Therefore, successfully run the docker-compose build command before the Kubernetes COP Web Server deployment.

The installation and setup instructions remain the same for the other services and should not involve or conflict with the Kubernetes COP deployment.
...
```bash
    ...
    ff02::3 ip6-allhosts
```
After updating the /etc/hosts, surf to the standard: https://local-dsp.virtru.com:5001/
...
## Turn Off Pods
kubectl scale deployment cop-web-server --replicas=0
## Turn On Pods
kubectl scale deployment cop-web-server --replicas=1
## Get Pods
kubectl get pods -w
## Get Pods IP
kubectl get svc cop-web-server
## Surf to COP Webpage
https://local-dsp.virtru.com:5001/