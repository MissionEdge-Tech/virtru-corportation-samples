...
```bash
docker compose --env-file env/default.env -f docker-compose.common.yaml up --build --force-recreate
```


**Stop the environment:**

The following will stop the enviroment and DSP application. Crtl + c in the terminal will also stop the containers however it is recommended
to also run the following down command as it will cleanup the container remnants.

```bash
docker compose --env-file env/default.env -f docker-compose.common.yaml down
```
